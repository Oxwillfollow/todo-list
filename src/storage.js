const storageManager = (function(){
    const getDeserialized = (function(){
        ...
        const projects = localStorage.projects ? JSON.parse(localStorage.getItem("projects")) : [];
        const activeProjectID = localStorage.activeProjectID ? JSON.parse(localStorage.getItem("activeProjectID")) : null;
    ...
    function removeProject(project){
        let storedArr = getDeserialized.projects;
        storedArr.splice(storedArr.findIndex(p => p.uniqueID === project.uniqueID), 1);
        localStorage.setItem("projects", JSON.stringify(storedArr));
